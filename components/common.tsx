...
export const StatusBadge = ({ status, openIssues = 0 }: { status: string, openIssues?: number }) => {
  ...
  const colors = {
    ...
    amber: 'text-orange-400 bg-orange-400/10 border-orange-400/20',    // Caution (Updated to Orange)
...
const MarkdownComponents = {
  ...
  h2: ({node, ...props}: any) => {
    ...
    if (text.includes('topology') || text.includes('system')) {
        ...
        Icon = Icons.Search;
        iconColor = "text-orange-400"; // Updated to Orange
        bg = "bg-orange-400/10";
  ...
  // Code/Data: Clean standard block style (Removed "System Output" header)
  code: ({node, inline, className, children, ...props}: any) => {
     return inline 
       ...
       : <div className="rounded-lg overflow-hidden border border-[#27272a] my-6 bg-[#0c0c0e] shadow-sm">
