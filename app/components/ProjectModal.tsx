...
const MetricCard = ({ label, value, subtext, icon, health }: { label: string, value: string, subtext: React.ReactNode, icon: React.ReactNode, health: HealthStatus }) => {
    ...
    return (
        <div className="p-4 rounded-lg border border-[#27272a] bg-[#121214] hover:border-[#3f3f46] transition-colors relative overflow-hidden group">
            <div className={`absolute top-0 right-0 p-24 ${health === 'green' ? 'bg-emerald-500' : health === 'amber' ? 'bg-amber-400' : health === 'red' ? 'bg-rose-500' : health === 'blue' ? 'bg-blue-500' : 'bg-gray-500'} opacity-[0.03] rounded-full -mr-10 -mt-10 pointer-events-none group-hover:opacity-[0.06] transition-opacity`}></div>
                ...
                <div className="h-20 border-b border-[#27272a] bg-[#121214] flex items-center justify-between px-8 shrink-0">
                    <div className="flex items-center gap-5">
                        <div className={`w-12 h-12 bg-gradient-to-br from-[#121214] to-[#18181b] rounded-xl border border-[#27272a] flex items-center justify-center shadow-lg shadow-black/20 ${
                            projectHealth === 'green' ? 'text-emerald-400' : projectHealth === 'amber' ? 'text-amber-400' : projectHealth === 'red' ? 'text-rose-500' : 'text-blue-400'
                        }`}>
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                                    ...
                                    projectHealth === 'amber' ? 'bg-amber-400/10 text-amber-400 border-amber-400/20' : 
                        ...
                        <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                <MetricCard 
                                    ...
                                    subtext={<><span className={projectHealth === 'red' ? 'text-rose-500' : projectHealth === 'amber' ? 'text-amber-400' : 'text-emerald-500'}>
                                ...
                                <MetricCard 
                                    ...
                                    value={project.progress > 50 ? "87%" : "N/A"} 
                                    subtext={project.progress > 50 ? <><span className="text-emerald-500">Pass</span> ASHRAE 62.1</> : <>Insufficient Data</>}
                                    icon={<Icons.CheckCircle />}
                                    health={project.progress > 50 ? 'green' : 'gray'}
                                />
                                <MetricCard 
                                ...
                                <MetricCard 
                                    ...
                                    subtext={project.openIssues > 0 ? <><span className={project.openIssues > 3 ? 'text-rose-500' : 'text-amber-500'}>{project.openIssues > 3 ? 'Immediate management attention' : 'Requires review'}</span></> : <>All clear</>}
